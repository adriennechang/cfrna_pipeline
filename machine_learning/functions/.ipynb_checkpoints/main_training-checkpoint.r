...
READ_DATA <- function(group1_name,
    ...
    meta_data <- read.delim(meta_data_path) %>% 
            filter(.data[[group_column]] %in% c(group1_vals,group2_vals)) %>% 
                    mutate(group = ifelse(.data[[group_column]] %in% group1_vals, !!group1_name, ifelse(.data[[group_column]] %in% group2_vals, !!group2_name, "WRONG SAMPLE")),
                            sample_id = .data[[id_column]],
                            sample_group_granular = .data[[group_column_granular]]) %>%
                    select(sample_id, group, sample_group,sample_group_granular) %>%
            filter(!is.na(sample_id)) %>%
            unique()


    meta_data$group <- factor(meta_data$group, levels = c(group1_name,group2_name))
    ...
    count_matrix <- read.delim(count_matrix_path,row.names = 1) %>%                                    ## read in count matrix
                    select(all_of(meta_data$sample_id)) %>% 
                    data.frame()
    ...
    count_matrix <- count_matrix[,meta_data$sample_id]                                                 ## Synchronize count and meta data frames
...
SPLIT_DATA <- function(count_matrix,
    ...
    meta_data_train <- meta_data %>% group_by(sample_group_granular) %>% slice_sample(prop=0.7)

    ## evenly distribute those with just one sample group
    single_groups <- meta_data %>% group_by(sample_group_granular) %>% dplyr::count() %>% filter(n == 1) %>% pull(sample_group_granular)
    more_train <- meta_data %>% filter(sample_group_granular %in% single_groups) %>% group_by(group) %>% slice_sample(prop = 0.7)
    meta_data_train <- rbind(meta_data_train, more_train)

    ## create test data based on left overs
    meta_data_test <- meta_data %>% filter(!(sample_id %in% meta_data_train$sample_id))
    
    ## Subset count matrixs
    count_matrix_train <- count_matrix[,meta_data_train$sample_id]
    count_matrix_test <- count_matrix[,meta_data_test$sample_id]
