...
MAIN <- function(group1_name,
    ...
    feature_selection_list <- FEATURE_SELECTION_NORMALIZATION(count_matrix_train,
                                                ...
                                                group2_name,
                                                NGENES)
    tof_noplatelet = read.delim("../analysis/agg_tissue_of_origin.txt")
    count_matrix_train_sig = feature_selection_list[["count_matrix_train_sig"]]
    count_matrix_train_sig = merge(count_matrix_train_sig, tof_noplatelet, by.x="row.names", by.y="sample_id")
    count_matrix_train_sig = count_matrix_train_sig[, colnames(count_matrix_train_sig) != "Cornell_ID"]
    rownames(count_matrix_train_sig) = count_matrix_train_sig$Row.names
    count_matrix_train_sig = count_matrix_train_sig[,-1]
    count_matrix_test_sig = feature_selection_list[["count_matrix_test_sig"]]
    count_matrix_test_sig = merge(count_matrix_test_sig, tof_noplatelet, by.x="row.names", by.y="sample_id")
    count_matrix_test_sig = count_matrix_test_sig[, colnames(count_matrix_test_sig) != "Cornell_ID"]
    rownames(count_matrix_test_sig) = count_matrix_test_sig$Row.names
    count_matrix_test_sig = count_matrix_test_sig[,-1]
...
READ_DATA <- function(group1_name,
 ...
 #meta_data = meta_data[meta_data$HIV.status=="Negative",]
    ...
    cols = intersect(colnames(read.delim(count_matrix_path,row.names=1)), meta_data$Cornell_ID)
