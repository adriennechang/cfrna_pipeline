...
### FIGURE 2B
```{r}
ggroc(list("train" = roc(df_final$Microbiologic.reference.standard, df_final$score), "test" = roc(df_val$Microbiologic.reference.standard, df_val$score)), legacy.axes=T) 
...
### FIGURE 2D
```{r}
newdf = counts[,colnames(counts) %in% best_genes] %>% mutate_all(as.numeric) %>% mutate( log2( . +1)) %>% data.frame() %>% t() %>% data.frame()
