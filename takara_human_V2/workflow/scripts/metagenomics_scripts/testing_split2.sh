...
echo "pigz -dc -p $threads $R1 | sort --parallel=$threads -T $dir > $dir/$basename.R1.outfmt6.sorted; cut -d'_' -f1 $dir/$basename.R1.outfmt6.sorted | cut -f1 | sort -u > $dir/$basename.R1.readids" >> $dir/prompt
echo "pigz -dc -p $threads $R2 | sort --parallel=$threads -T $dir > $dir/$basename.R2.outfmt6.sorted; cut -d'_' -f1 $dir/$basename.R2.outfmt6.sorted | cut -f1 | sort -u > $dir/$basename.R2.readids" >> $dir/prompt

parallel -j $threads < $dir/prompt

comm -12 <(sort $dir/$basename.R1.readids) <(sort $dir/$basename.R2.readids) > $dir/READIDS
