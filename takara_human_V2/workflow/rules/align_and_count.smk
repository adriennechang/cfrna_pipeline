...
rule filter_q30:
    ...
    shell:
        """
        if [ {params.lib_prep} = "ULTRA_II_DIR" ]
        then
             {params.BBDUK} -Xmx1g in1={input.r1} in2={input.r2} out1={output.r1_clean} out2={output.r2_clean} ref={params.ref_apt} ktrim=r k=21 mink=11 hdist=2 qtrim=rl trimq=10 minlen=11 tpe tbo >& {params.bbduk_output}
        fi
...
rule rename_headcrop_repair:
    ...
    shell:
        ...
        if [ {params.lib_prep} = "ULTRA_II_DIR" ]
        then
	     umi_tools extract --log {params.log_loc} --bc-pattern=NNNNNNNNNNN \
	     -I {input.r2_clean} -S {output.r2_named} \
	     --read2-in={input.r1_clean} --read2-out={output.r1_named}
